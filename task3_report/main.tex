...
Subject\$StringAlgorithms    & 196 & 101\;\;(51.5\%) \\
Subject\$DateTimeAlgorithms  & 159 & 50\;\;(31.4\%) \\
Subject\$GamePlace           & 31  & 9\;\;(29.0\%) \\
Subject\$GamePlayer          & 25  & 0\;\;(0.0\%) \\
Subject\$GameConfiguration   & 101 & 0\;\;(0.0\%) \\
\midrule
\textbf{Overall}             & \textbf{513} & \textbf{161\;\;(31.4\%)} \\
\bottomrule
\end{tabular}
...
For the advanced prompt we added three things: a few-shot example, a chain-of-thought instruction
asking the model to enumerate boundary conditions step by step before writing each test,
and inline clarifications to the docstrings. Importantly, we did NOT give the model any
function bodies---all clarifications were derived from reading the docstrings carefully
(and in a couple cases checking the actual behavior manually to confirm our reading).

\subsubsection*{Interaction Record --- Advanced Prompt}
...
Here's a worked example so you know the expected style:
...
$ java -cp ... org.junit.runner.JUnitCore comp5111.assignment.cut.LLMAdvancedTest
...
\midrule
Subject (outer)              & 1   & 100.0\% \\
Subject\$StringAlgorithms    & 196 & 120\;\;(61.2\%) \\
Subject\$DateTimeAlgorithms  & 159 & 66\;\;(41.5\%) \\
Subject\$GamePlace           & 31  & 12\;\;(38.7\%) \\
Subject\$GamePlayer          & 25  & 0\;\;(0.0\%) \\
Subject\$GameConfiguration   & 101 & 0\;\;(0.0\%) \\
\midrule
\textbf{Overall}             & \textbf{513} & \textbf{199\;\;(38.8\%)} \\
\bottomrule
\end{tabular}
...
\midrule
Simple   & 41 & 3 & 31.4\% & 51.5\% & 31.4\% \\
Advanced & 65 & 0 & 38.8\% & 61.2\% & 41.5\% \\
\textbf{$\Delta$} & \textbf{+24} & \textbf{$-3$} & \textbf{+7.4\%} & \textbf{+9.7\%} & \textbf{+10.1\%} \\
\bottomrule
\end{tabular}
...
we found five recurring patterns that make the current docstrings hard to test from.
...
\begin{table}[h]
\centering
\begin{tabular}{p{4.2cm}ccp{3.8cm}}
\toprule
\textbf{Issue} & \textbf{Failures} & \textbf{Cov.\ gap} & \textbf{Root cause} \\
\midrule
Wrong example (\texttt{extractIntInStr})   & 1 & yes & Factual error in doc \\
Terminator ambiguity (\texttt{parseToken}) & 2 & yes & Ambiguous word choice \\
No format (\texttt{roundSecondToString})   & 0 & yes & Missing format spec \\
Null elements (\texttt{trimArrayElements}) & 0 & yes & Unstated null handling \\
Sentinel priority (\texttt{strToBoolean})  & 0 & yes & Missing priority order \\
\bottomrule
\end{tabular}
\caption{Summary of specification issues}
\end{table}
...
a handful of verified input/output pairs. ``Verified'' here means actually running the function
(or tracing the code) to confirm the example is correct---not just writing down what you
think it should be.
...
This can actually be automated. An LLM given the function body can generate and verify
examples:
\begin{promptbox}
\textbf{Automation prompt:} Given this Java function body, generate 6--8 diverse examples
covering null, empty, all-digits, stops-at-non-digit, and leading-zeros cases. Trace the
code mentally, and output only verified \texttt{input -> output} pairs.
\end{promptbox}
...
This is another good candidate for automation---an LLM with the function body can
...
We enhanced the docstrings of five functions (\texttt{extractIntInStr},
...
We then prompted GPT-5.2 with only the enhanced docstrings (still no function bodies) to
...
$ java -cp ... org.junit.runner.JUnitCore comp5111.assignment.cut.LLMEnhancedTest
...
and the most automatable change is having an LLM generate verified examples from the function body.
